...
int _matriz_qtd_total_elementos(Matriz *m) {
    int qtd = 0;
    for (int i = 0; i < m->line_head_size; i++)
    {
        Celula *c = m->line_head[i]->head;
    ...
    return qtd;
    /* Por ter duas iteracoes uma dentro da outra
        A complexidade dessa funcao eh de O(n^2)
    */
}

void matriz_create_bynarie_arq(Matriz *m) {
    int qtd_elem = _matriz_qtd_total_elementos(m);
    FILE *arq;
    arq = fopen("Matriz_binario.bin", "wb");

    fwrite(&m->line_head_size, sizeof(int), 1, arq);
    fwrite(&m->colune_head_size, sizeof(int), 1, arq);
    fwrite(&qtd_elem, sizeof(int), 1, arq);
    for (int i = 0; i < m->line_head_size; i++)
    {
        Celula *c = m->line_head[i]->head;
        while (c != NULL)
        {
            fwrite(&c->line, sizeof(int), 1, arq);
            fwrite(&c->col, sizeof(int), 1, arq);
            fwrite(&c->val, sizeof(float), 1, arq);
            c = c->next_col;
        }
        
    }
    fclose(arq);
    /*  A complexidade dessa funcao seria O(n^2) pois dentro de uma busca nas linhas
        temos um while de varredura nas colunas, multiplicando o n
    */
}

Matriz *matriz_read_bynarie_arq() {
    int linha = 0, coluna = 0, qtd_elem = 0;
    FILE *arq;
    arq = fopen("Matriz_binario.bin", "rb");
    fread(&linha, sizeof(int), 1, arq);
    fread(&coluna, sizeof(int),1, arq);
    Matriz *m_new = matriz_create(linha, coluna);
    fread(&qtd_elem, sizeof(int), 1, arq);

    for (int i = 0; i < qtd_elem; i++)
    {
        int lin_atual, col_atual;
        float val_atual;
        fread(&lin_atual, sizeof(int), 1, arq);
        fread(&col_atual, sizeof(int), 1, arq);
        fread(&val_atual, sizeof(float), 1, arq);
        matriz_set_value(m_new, lin_atual, col_atual, val_atual, ATUALIZAR);
    }
    
    fclose(arq);
    return m_new;
    /*
        A complexidade dessa funcao de leitura, levando em consideracao a 
        funcao de set_value, seria de O(n^2), pois esta dentro do for.
    */
}
